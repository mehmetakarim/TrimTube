...
**İKİNCİ PLAN DÖNEMİ (15 Tem 2026):** Kullanıcının 16 maddelik özellik notları + saha/kod analizi birleştirilerek Faz 11–16 + kulvarlar `YOL-HARITASI.md`'ye yazıldı (öncelik mantığı ve kapsam kararları orada). Önemli kararlar: Content ID simülasyonu yapılamaz → "reklam dostu" uyarısına daraltıldı; notarization Apple Developer ID şartına bağlı beklemede; tarayıcı eklentisi `extension/` klasöründe ayrı kulvar.
...
**Faz 15 — Moodlar & AI Director (zirve özellik): KOD TAMAM (16 Tem 2026, Windows) — GERÇEK ANAHTARLARLA SAHA TESTİ ve v1.15.0 YAYIN KARARI BEKLİYOR (yalnız yerel commit).**
- Yeni **"Moodlar" ekranı** (sol nav 6. öğe): bölüm dosyası → mood (Komedi/Dram/Gerilim/Duygusal/Özet, `MOODS` ton tarifleri) + hedef süre (30/60/90) + anlatıcı sesi → **"Kurgu planı oluştur"** (Whisper + Gemini; plan ekranda sahne listesi olarak önizlenir) → **"Seslendir ve Montajla"** (ElevenLabs TTS + montaj). Yol haritasındaki "Faz 9 kuyruğunda" fikri yerine bilinçli olarak bağımsız-ekran deseni: kendi `mood-progress` kanalı, `runMoodProc`/`moodAbort` iptali; kuyrukla eşzamanlı çalışır.
